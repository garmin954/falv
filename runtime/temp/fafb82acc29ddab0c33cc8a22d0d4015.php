...
<link rel="StyleSheet" href="/public/static/vendor/tree/css/dtree.css" type="text/css" />
<script type="text/javascript" src="/public/static/vendor/tree/js/dtree.js?v=513213"></script>
<style>
...
<div class="x-body layui-col-md3 layui-col-lg3 layui-col-sm3  layui-col-xs3 " style="background: #f3f5e5;overflow-y: scroll;max-height: 490px;" >
    <div class="dtree">
        ...
        <script type="text/javascript">
            ...
            d.add(<?php echo $rule['id']; ?>,<?php echo $rule['pid']; ?>,'authority','<?php echo $rule['id']; ?>','<?php echo $rule['title']; ?>@yl');
            <?php endforeach; endif; else: echo "" ;endif; ?>

            document.write(d);
                    d.openAll();

                    <?php if(is_array($rule_top) || $rule_top instanceof \think\Collection || $rule_top instanceof \think\Paginator): $i = 0; $__LIST__ = $rule_top;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$top): $mod = ($i % 2 );++$i;?>
                    d.openTo(<?php echo $top['id']; ?>,true);
...
<div class="x-body layui-col-md9 layui-col-lg9 layui-col-sm9  layui-col-xs9" >
    ...
    <xblock>
        ...
        <button class="layui-btn" type="button" onclick="x_admin_show('添加规则','<?php echo url('admin/auth_rule/add'); ?>',650,400)"><i
    ...
    <table class="layui-table">
        ...
        <tr pid="<?php echo $vo['pid']; ?>" mid="<?php echo $vo['id']; ?>" <?php if($vo['level'] != 0): ?>style="display:none; background: lightgoldenrodyellow;"<?php endif; ?> >
        ...
        <td><i class="iconfont"><?php echo $vo['icon']; ?></i>&nbsp;
            <?php if($vo['level'] > 0): ?>|<?php endif; ?>
            <?php echo str_repeat('----+',$vo['level']);?>
            <?php echo $vo['title']; ?>

            <a style="float: right" title="添加子规则"
                           onclick="x_admin_show('添加规则','<?php echo url('admin/auth_rule/add',array('pid'=>$vo['id'])); ?>',650,400)"
                           href="javascript:;">
            <i class="iconfont">&#xe600;</i>
        </a></td>
        <td>
...
<td class="td-manage">
    ...
    <a title="编辑" onclick="x_admin_show('修改规则','<?php echo url('admin/auth_rule/edit',array('id'=>$vo['id'])); ?>',650,400)"
       href="javascript:;">
